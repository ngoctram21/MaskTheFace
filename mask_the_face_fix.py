...
parser.add_argument(
    ...
    type=str,
    default="",
    help="Path to either the folder containing data",
)
parser.add_argument(
    "--mask_type",
    type=str,
...
# read csv meta
with open(args.csv_path, 'r') as read_obj:
...
# Check file as empty
    if header != None:
        ...
        for row in tqdm(csv_reader):
            ...
            uid,filename = row[:2]
            image_path = args.root_path + uid + '/' + filename
#             print(f'image_path: {image_path}')
            list_landmark = row[2:]
            list_landmark = group([float(item) for item in list_landmark],2)
#             print(list_landmark)
    #         face_landmark = shape_to_landmarks(list_landmark)
            ...
            is_directory, is_file, is_other = check_path(image_path)
#             display_MaskTheFace()
            print(image_path)
    #         if is_directory:
    ...
    #                 write_path = args.write_path + '/' + f + "_masked"
    ...
    #             image_path = args.path
                write_path = args.des_path + uid + '/' 
                if is_image(image_path):
                    # Proceed if file is image
                    # masked_images, mask, mask_binary_array, original_image
    #                 image = cv2.imread(image_path)
                    face_locations = [[0,0,112,112]]
    #                 six_points_on_face, angle = get_six_points(face_landmarks, image)
                    masked_image, mask, mask_binary_array, original_image = mask_image(
                        image_path,face_locations,list_landmark, args
                    )
                    for i in range(len(mask)):
#                         print(f'SHAPE: {masked_image[0].shape}')
                        if not os.path.exists(write_path):
                            os.makedirs(write_path)
                        w_path = write_path + filename.split('.')[0] + "_" + mask[i] + "." + filename.rsplit(".")[1]
#                         print(w_path)
                        img = masked_image[0]
#                         print(img)
                        cv2.imwrite(w_path, img)
